var eg_w1011 = { 
    ...
    ret_detalle_ini : async function  (xthis) {   
      ...
      try {
            ...
            return  {ok:true, key_find:key_com}
          } catch (error) {
    ...
    f_setvalues_head : async function  (key_com, filename){
      try {     
...
console.log('ca_w1020.js  ue_Aceptar previo ',sw_y , parametros);    
    let response =null;
    if (sw_y=='M'){
...
console.log('ca_w1020.js  ue_Aceptar result ',resultado);
    if (!response.ok) {
...
console.log('f_genera_json_upd ca_w1020 ',$form);  
